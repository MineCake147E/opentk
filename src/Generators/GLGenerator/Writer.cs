...
namespace GLGenerator
{
    internal static class Writer
    {
        ...
        private static void WriteOverloadMethod(IndentedTextWriter writer, Overload overload, bool postfixNativeCall)
        {
            ...
            string parameterString = string.Join(", ", overload.InputParameters.Select(p => p.ToDefinitionString()));
        ...
        private static string? WriteNestedOverload(IndentedTextWriter writer, Overload overload, NameTable nameTable, bool postfixNativeCall)
        {
            ...
            else
            {
                ...
                string arguments = string.Join(", ", nativeFunction.Parameters.Select(p =>
                {
                    var sb = new StringBuilder();
                    if (p.StrongType is CSRef csRef)
                    {
                        _ = sb.Append(csRef.GetCallModifiers());
                        _ = sb.Append(' ');
                    }
                    _ = sb.Append(nameTable[p]);
                    return sb.ToString();
                }));
